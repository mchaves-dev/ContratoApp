...
public sealed class ContratoApiClient(HttpClient httpClient)
{
	...
	public async Task<ApiResult<ClienteResponse>> AtualizarClienteAsync(Guid id, AtualizarClienteRequest request, CancellationToken cancellationToken = default)
		=> await PutAsync<AtualizarClienteRequest, ClienteResponse>($"/api/clientes/{id}", request, cancellationToken);
